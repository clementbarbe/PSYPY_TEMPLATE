"""
Stroop Color-Word Task (French) — blocked fMRI.

Consigne :
    Repondre a la COULEUR DE L'ENCRE, jamais au mot.
    3 couleurs : rouge, bleu, vert
    3 conditions :
        Congruent   : ROUGE en rouge, BLEU en bleu, VERT en vert
        Incongruent : ROUGE en bleu, ROUGE en vert, BLEU en rouge, etc.
        Neutre      : XXXX / %%%% / OOOO en couleur

Timing par essai (2s total) :
    500ms fixation + 1200ms stimulus + 300ms ITI

Rappel touches affiche en permanence en bas de l'ecran.

Touches PC :
    Gauche = ROUGE    Bas = BLEU    Droite = VERT

Randomisation intra-bloc :
    - Jamais >2 essais consecutifs meme couleur
    - Jamais >2 essais consecutifs meme mot
    - Equilibre exact des 3 couleurs
    - Blocs incongruents : 6 paires equilibrees
"""
...
class StroopTask(BaseTask):
    """Stroop Color-Word fMRI task (French, 3 colors)."""
    ...
    def _setup_stimuli(self) -> None:
        ...
        # Incongruent pairs (balanced)
        pairs_cfg = self.task_config.get('incongruent_pairs', [])
        if pairs_cfg:
            self._inc_pairs = [(p['word'], p['ink']) for p in pairs_cfg]
        else:
            self._inc_pairs = []
            for w in self._ink_colors:
                for i in self._ink_colors:
                    if w != i:
                        self._inc_pairs.append(
        ...
        # Key reminder (always visible at bottom)
        show_reminder = self.task_config.get('show_key_reminder', True)
        if show_reminder:
            rh = self.task_config.get('key_reminder_height', 0.04)
            ry = self.task_config.get('key_reminder_y', -0.85)

            k_r = self._color_to_key['rouge'].upper()
            k_b = self._color_to_key['bleu'].upper()
            k_v = self._color_to_key['vert'].upper()

            self._reminder_stims = []

            # Rouge
            self._reminder_stims.append(visual.TextStim(
                self.win, text=f"[{k_r}] ROUGE",
                color=self._color_rgb['rouge'],
                height=rh, pos=(-0.35, ry), bold=True, font='monospace',
            ))
            # Bleu
            self._reminder_stims.append(visual.TextStim(
                self.win, text=f"[{k_b}] BLEU",
                color=self._color_rgb['bleu'],
                height=rh, pos=(0.0, ry), bold=True, font='monospace',
            ))
            # Vert
            self._reminder_stims.append(visual.TextStim(
                self.win, text=f"[{k_v}] VERT",
                color=self._color_rgb['vert'],
                height=rh, pos=(0.35, ry), bold=True, font='monospace',
            ))
        else:
            self._reminder_stims = []
    ...
    def _draw_reminder(self):
        """Draw key reminder at bottom of screen."""
        for s in self._reminder_stims:
            s.draw()
    ...
    def _get_instruction_text(self) -> str:
        ...
        return (
            ...
            f"Les rappels restent affiches en bas de l'ecran.\n\n"
    ...
    def generate_trials(self, block_def: dict) -> list:
        """
        Generate balanced trial list for one block.

        Returns list of (word, ink_color, trial_type) tuples.

        Constraints:
            - 3 colors equally represented
            - No >2 same color in a row
            - No >2 same word in a row
        ...
        # Double desequence: on color AND on word
        trials = self._double_desequence(trials)
        return trials

    def _gen_congruent(self, n: int) -> list:
        """Balanced congruent: each color equally often."""
        trials = []
        per_color = n // 3
        remainder = n - per_color * 3

        for c in self._ink_colors:
            count = per_color + (1 if remainder > 0 else 0)
            remainder = max(0, remainder - 1)
            word = self._color_words[c]
            trials.extend([(word, c, 'congruent')] * count)

        random.shuffle(trials)
        return trials

    def _gen_incongruent(self, n: int) -> list:
        """Balanced incongruent: 6 pairs equally distributed."""
        trials = []
        n_pairs = len(self._inc_pairs)
        per_pair = n // n_pairs
        remainder = n - per_pair * n_pairs

        for i, (word, ink) in enumerate(self._inc_pairs):
            count = per_pair + (1 if i < remainder else 0)
            trials.extend([(word, ink, 'incongruent')] * count)

        random.shuffle(trials)
        return trials

    def _gen_neutral(self, n: int) -> list:
        """Balanced neutral: 3 colors equally, cycling symbols."""
        trials = []
        per_color = n // 3
        remainder = n - per_color * 3
    ...
    def _double_desequence(self, trials: list,
                           max_consecutive: int = 2) -> list:
        """
        Ensure no >max_consecutive same color AND no >max_consecutive same word.
        """
        # First pass: desequence on ink color
        trials = desequence(
            trials, key_func=lambda t: t[1],
            max_consecutive=max_consecutive,
        )
        # Second pass: check words, swap if needed
        for attempt in range(50):
            ok = True
            for i in range(max_consecutive, len(trials)):
                window = [trials[j][0] for j in range(
                    i - max_consecutive, i + 1
                )]
                if len(set(window)) == 1:
                    ok = False
                    # Swap with a random later position
                    swap_candidates = [
                        k for k in range(i + 1, len(trials))
                        if trials[k][0] != trials[i][0]
                    ]
                    if swap_candidates:
                        swap_idx = random.choice(swap_candidates)
                        trials[i], trials[swap_idx] = (
                            trials[swap_idx], trials[i]
                        )
                    break
            if ok:
                break
    ...
    def _print_task_stats(self) -> None:
        ...
        if len(by_ink) > 1:
            ...
            for c in sorted(by_ink):
                ...
                n = len(recs)
                acc = 100 * sum(r['is_correct'] for r in recs) / n
                rts = [r['rt'] for r in recs
                       if r['rt'] > 0 and r['is_correct']]
